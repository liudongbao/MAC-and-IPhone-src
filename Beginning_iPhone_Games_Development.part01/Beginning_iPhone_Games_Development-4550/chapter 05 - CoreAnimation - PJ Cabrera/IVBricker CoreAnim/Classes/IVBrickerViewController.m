...
- (void)startPlaying {
	...
	[hitBricks removeAllObjects];
	[hitBricks release];
	hitBricks = [[NSMutableSet alloc] init];
...
- (void)processCollision:(UIImageView *)brick 
{
	...
	brick.alpha -= 0.1;
	[hitBricks addObject:brick];

	[UIView beginAnimations:nil context:nil];
	[UIView setAnimationDuration:1];
	
	for (UIView *aBrick in [hitBricks objectEnumerator]) {
		
		aBrick.alpha = 0;
		CGRect newFrame = aBrick.frame;
		newFrame.origin = CGPointMake(newFrame.origin.x, 500);
		aBrick.frame = newFrame;
	}
