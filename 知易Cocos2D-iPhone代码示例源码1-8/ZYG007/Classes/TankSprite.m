...
- (void) OnFire:(CCLayer *)layer
{
	...
	switch (kAct) {
		case kUp:
			ptFrom = ccp(self.position.x, self.position.y  + self.textureRect.size.height / 2);
		...
		case kDown:
			...
			break;
		case kRight:
			ptFrom = ccp(self.position.x  + self.textureRect.size.width / 2, self.position.y);
	...
	[bullet FireFrom:ptFrom To:ptTgt bulletType:bt];
